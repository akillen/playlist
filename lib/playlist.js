...
var makeRecursive = function (workingDir) {
  console.log('Searching: %s.', workingDir);
...
function makeM3u(files, dir, workingDir) {
  if (dir.findIndex(isM3u) >= 0) {
    console.log('%s: m3u found.', workingDir);
  } else {
    console.log(workingDir);
    var filename = workingDir.split('/').slice(-1)[0] + '.m3u';
    console.log(filename);
    fs.writeFile(path.join(workingDir, filename), files.join('\n'),
      err => {
        if (err) {
          console.log(err);
        } else {
